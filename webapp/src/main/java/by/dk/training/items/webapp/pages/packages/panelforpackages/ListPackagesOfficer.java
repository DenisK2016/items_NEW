...
public class ListPackagesOfficer extends Panel {

	private static final long serialVersionUID = 1L;
	@Inject
	private PackageService packageService;

	public ListPackagesOfficer(String id) {
		super(id);
	...
	protected void onInitialize() {
		...
		modal1.setWindowClosedCallback(new WindowClosedCallback() {
			...
			public void onClose(AjaxRequestTarget target) {
				target.add(ListPackagesOfficer.this);
		...
		DataView<Package> dataView = new DataView<Package>("packagelist", dataProvider, 5) {
			...
			protected void populateItem(Item<Package> item) {
				...
				Link update = new Link<PackRegPage>("updatePack") {
				...
				item.add(update);
				if (AuthorizedSession.get().getUser().getId() != pack.getIdUser().getId()) {
